...
{
#ifdef DLL
	void *handle = dlopen("./liblibrary.so", RTLD_LAZY); //open library

	char**(*create_table)(int, int) = dlsym(handle, "create_table");
	void(*delete_table)(char**, int) = dlsym(handle, "delete_table");
    void(*add_block)(char**, int) = dlsym(handle, "add_block");
    void(*delete_block)(char**, int) = dlsym(handle, "delete_block");
    int(*find_block)(char**, int, int, int) = dlsym(handle, "find_block");
    ...
    int(*s_find_block)(int, int, int) = dlsym(handle, "s_find_block");
#endif
	////////////////////////////////////////////////////////////////////
	if (argc < 4)
	{
    ...
    if (strcmp("static", argv[3]) == 0)
    {
        if(strcmp("c", argv[4]) == 0) //create table
        {
            ...
            printf("\nUser time: %ld \nSystem time: %ld \nReal time: %ld s %ld ns\n",
                ...
                (stop_real->tv_sec-start_real->tv_sec),(stop_real->tv_nsec-start_real->tv_nsec));
        ...
        for(int i = 5 ; i < argc ; i++)
        {
            if(strcmp("f", argv[i]) == 0) //find element
            {
                ...
                printf("Found index: %i", s_find_block(atoi(argv[++i]), elements, size));
            ...
            else if(strcmp("ad", argv[i]) == 0) //add and delete blocks
            {
                ...
                fprintf(f, "Delete and add blocks");
                fprintf(f, "\nUser time: %ld \nSystem time: %ld \nReal time: %ld s %ld ns\n\n",
                stop->tms_utime-start->tms_utime,
                stop->tms_stime-start->tms_stime,
                (stop_real->tv_sec-start_real->tv_sec),(stop_real->tv_nsec-start_real->tv_nsec));
            ...
            else if(strcmp("adc", argv[i]) == 0) //add and delete blocks
            {
                ...
                fprintf(f, "Delete and add blocks - cross");
                fprintf(f, "\nUser time: %ld \nSystem time: %ld \nReal time: %ld s %ld ns\n\n",
                stop->tms_utime-start->tms_utime,
                stop->tms_stime-start->tms_stime,
                (stop_real->tv_sec-start_real->tv_sec),(stop_real->tv_nsec-start_real->tv_nsec));
    ...
    else if (strcmp("dynamic", argv[3]) == 0)
    {
        ...
        for(int i = 5 ; i < argc ; i++)
        {
            if(strcmp("f", argv[i]) == 0) //find element
            {
                ...
                printf("Found index: %i", find_block(table, elements, size, atoi(argv[++i])));
