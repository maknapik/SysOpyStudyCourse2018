...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        if (shm->clients == 0)
        {
            ...
            semop(semID, &waitSem, 1); //czekanie az ktos obudzi golibrode
            shm->asleep = 0;
            unblockSem.sem_num = ASLEEP;
            semop(semID, &unblockSem, 1); //klient blokuje flage, gdy golibroda spi, wiec zmieniamy
            printf("Time: %ld\tGOLIBRODA: Waking up.\n", getTime());
            unblockSem.sem_num = SIT;
            semop(semID, &unblockSem, 1); //klient siada
            waitSem.sem_num = SHAVE;
            semop(semID, &waitSem, 1); //czekanie az klient usiadzie
            ...
            semop(semID, &unblockSem, 1); //klient odchodzi
            waitSem.sem_num = LEFT;
            semop(semID, &waitSem, 1); //czekanie az klient wyjdzie
        } 
        else
        {
            ...
            waitSem.sem_num = SHAVE;
            semop(semID, &waitSem, 1); //czekanie az usiadzie
            printf("Time: %ld\tGOLIBRODA: Shaving client with PID: %d\n", getTime(), shm->currShaved);
            printf("Time: %ld\tGOLIBRODA: Ended shaving client with PID: %d\n", getTime(), shm->currShaved);
            unblockSem.sem_num = ENDSHAVE;
