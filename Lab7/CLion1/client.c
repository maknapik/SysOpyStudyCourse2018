    ...
    for(int i = 0 ; i < clients ; i++)
    {
        ...
        if(client == 0)
        {
            for(int j = 0 ; j < shaves ; j++)
            {
                ...
                semop(semID, &waitSem, 1); //czekanie na mozliwosc sprawdzenia czy golibroda spi
                if(shm->asleep == 1)
                {
                    shm->currShaved = getpid();

                    printf("Time: %ld\tClient: %d\twaking up barber.\n", getTime(), getpid());
                    unblockSem.sem_num = WAKEUP; //budzenie golibrody
                    semop(semID, &unblockSem, 1);
                    waitSem.sem_num = SIT;
                    semop(semID, &waitSem, 1); //czekanie na pozowlenie zeby usiasc
                    printf("Time: %ld\tClient: %d\tsitting on barber's chair.\n", getTime(), getpid());
                    unblockSem.sem_num = SHAVE; //strzyzenie
                    semop(semID, &unblockSem, 1);
                    waitSem.sem_num = ENDSHAVE; //czekanie na koniec strzyzenia
                    semop(semID, &waitSem, 1);
                    printf("Time: %ld\tClient: %d\tis shaved.\n", getTime(), getpid());
                    unblockSem.sem_num = LEFT;
                    semop(semID, &unblockSem, 1); //klient wychodzi
                }
                else
                {
